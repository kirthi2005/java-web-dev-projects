package org.launchcode;

import java.util.Scanner;
public class StringIndexLength {
    public static void main(String args[]){
            String sentence = "Alice was beginning to get very tired of sitting by her sister on the " +
                    "bank, and of having nothing to do: once or twice she had peeped into the book her " +
                    "sister was reading, but it had no pictures or conversations in it, ‘and what is " +
                    "the use of a book,’ thought Alice ‘without pictures or conversation?’";
            Scanner input = new Scanner(System.in);
            System.out.println("Enter your search string");
            String searchString = input.nextLine();
            Integer searchLength = searchString.length();
            System.out.println("Length of the search string: " + searchLength);
            Integer searchIndex = sentence.indexOf(searchString);
            System.out.println("Search string found at the index: " + searchIndex);
            String modifiedSentence = sentence.replace(searchString,"");
            System.out.println("Modified sentence: " + modifiedSentence);
    }
}


